...
CONTAINS  
!-----------------------------------------------------------------------------------------
  SUBROUTINE Init_Class_Fields(alfa_in,beta_in,Kdif_in,uvel_in,DeltaT_in)
    IMPLICIT NONE
      REAL (KIND=r8), INTENT(in) :: alfa_in
      REAL (KIND=r8), INTENT(in) :: beta_in
      REAL (KIND=r8), INTENT(in) :: Kdif_in
      REAL (KIND=r8), INTENT(in) :: uvel_in
      REAL (KIND=r8), INTENT(in) :: DeltaT_in
      INTEGER :: i,xb(1),xf(1)
      REAL (KIND=r8):: t
...
CONTAINS
 SUBROUTINE InitClass_WritetoGrads()
    IMPLICIT NONE
   FileName=''
   FileName='AdvecDiffusionLinearConceitual1D'
 ...
 FUNCTION SchemeWriteCtl(nrec)  RESULT (ok)
   ...
   ACCESS='SEQUENTIAL',STATUS='UNKNOWN',ACTION='WRITE')
    ...
    WRITE (UnitCtl,'(A           )')'ua 0 99 resultado da analiti'
 ...
 FUNCTION Upstream(Qm,Qc,Qp,t,dt,dx)  result(Qnp1)
   ...
   N1=UBOUND(Qc,DIM=1)
    IF( C>0 )THEN
        DO i=N0,N1
           CALL index(i,xb,xc,xf)
           Qnp1(i) = Qc(xc) - dt*C*( Qc(xc) - Qc(xb) )/dx + dt*Kdif*( Qm(xf) - 2*Qm(xc) + Qm(xb) )/(dx*dx)
        END DO
    ELSE
        DO i=N0,N1
           CALL index(i,xb,xc,xf)
           Qnp1(i) = Qc(xc) - dt*C*( Qc(xf) - Qc(xc) )/dx + dt*Kdif*( Qm(xf) - 2*Qm(xc) + Qm(xb) )/(dx*dx)
        END DO
    END IF
...
!************************************************************************************
 ...
 FUNCTION CentredSpace(Q,t,dt,dx)  result(rhs)
   ...
   DO i=N0,N1
      CALL index(i,xb,xc,xf)
      rhs(i) =  -( C*Q(xf) - C*Q(xb) )/(2*dx) +    Kdif*(Q(xf) -2*Q(xc) + Q(xb) )/(dx*dx)
   END DO
...
!   !************************************************************************************
 ...
 FUNCTION UpwindSpace(Q,t,dt,dx)  result(rhs)
   ...
   IF( C > 0.0 )THEN
      DO i=N0,N1
          CALL index(i,xb,xc,xf)
           rhs(i) =  - ( C*Q(xc) - C*Q(xb) )/dx + Kdif*(Q(xf) -2*Q(xc) + Q(xb) )/(dx*dx)
      END DO 
    ELSE
      DO i =N0,N1
          CALL index(i,xb,xc,xf)
          rhs(i) = -   ( C*Q(xf) - C*Q(xc))/dx + Kdif*(Q(xf) -2*Q(xc) + Q(xb) )/(dx*dx)
      END DO
    END IF
...
PROGRAM  Main
  ...
  USE Class_WritetoGrads, Only : SchemeWriteCtl,SchemeWriteData,InitClass_WritetoGrads
   ...
   CHARACTER(LEN=10)           :: scheme = 'CTCS'!'RK4CS'   ! Advection scheme. Possible values: UPSTREAM, CTCS,RK4CS,RK4CS_RA,RK4CS_RAW
   REAL(KIND=r8)               :: alfa   = 0.3_r8
   REAL(KIND=r8)               :: beta   = 0.6_r8    ! 0.5<  beta  <=1
   REAL(KIND=r8)               :: Kdif   = 2.0e-1_r8
   REAL(KIND=r8)               :: uvel   = 1.5_r8
   REAL(KIND=r8)               :: dt     = 0.2_r8
  ...
  SUBROUTINE Init()
      CALL Init_Class_Fields(alfa,beta,Kdif,uvel,dt)
  ...
  SUBROUTINE Run(irec_err,unit)
	  ...
	  u=up
          ...
          test=SchemeWriteData(u  ,irec)
          test=SchemeWriteData(ua ,irec)
          print*, 'iter [',nn,']  time [',t,']',MAXVAL(u),MAXVAL(ua)
        END DO
        test=SchemeWriteCtl(nn)

   PRINT*,'scheme=',TRIM(scheme),' err=',err/nn,' DeltaX=',DeltaX,' DeltaT=',DeltaT,' CFL=',C*DeltaT/DeltaX
